...
const { getProjectById, updateProjectDocument } = useProjectStore();
...
onMounted(async () => {
  ...
  currentFile.value = project.document;
...
const handleFileUpload = async () => {
  ...
  await updateProjectDocument(projectId.value, form);
};
</script>
    ...
    <div class="w-full mx-auto p-4 flex">
      ...
      <div class="w-1/4 p-4">
        <div class="overflow-x-hidden">
          ...
          <input @change="handleFileChange" id="document" name="document" type="file"
          ...
          <button @click="handleFileUpload" class="bg-blue-500 text-white p-2 mt-2 w-full rounded-lg">
            Upload
          </button>
          <div v-if="currentFile"
            class="mt-4 bg-green-500 text-white p-2 w-full rounded-lg text-center flex items-center justify-center">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 mr-2" viewBox="0 0 384 512">
              <path fill="#fff"
